...
在 React 中，使用 Props Spreading（物件展開運算符 `...`）是一種簡潔而強大的方法來傳遞大量的屬性。而在 Framer Motion 的開發中，這個技巧更能幫助我們有效地組織動畫物件，讓程式碼更可讀且易於維護。

這篇文章將介紹如何使用 Props Spreading 的方式，將動畫參數物件傳遞給 Framer Motion 的 `motion` 元件，並展示一些實用的應用場景。
...
[Motion](https://motion.dev/)（前身為 Framer Motion）的 `motion` 元件通常需要接收多個動畫屬性，例如 `initial`、`animate`、`exit` 等。如果這些屬性當中要調整的值又包含了透明度、背景顏色、x y 位置……等等的話整個元件 props 就會落落長，這時候我們就能將與動畫相關的屬性整合成一個物件，然後透過 Props Spreading 傳遞，讓程式碼更整潔。

原本的 `motion` 元件寫法：

```jsx
<motion.div className="size-10 bg-slate-500" initial={{ scale: 0, x: 0, y: 0 }} animate={{ scale: 2, x: 100, y: 200 }} transition={{ duration: 1 }} />
```
