...
export default async function LibraryPage({ searchParams }: { searchParams: Promise<{ type?: string; rating?: string }> }) {
  ...
  const filteredWorks = works.filter((item) => {
    const matchesType = item.type.toLowerCase() === currentType
        ...
        {filteredWorks.length === 0 ? (
          ...
          filteredWorks
            ...
            .map((work) => (
              <li key={work.id}>
                <a
                  className="bg-muted relative grid aspect-[2/3] place-items-center overflow-clip rounded-lg border p-2 shadow-sm"
